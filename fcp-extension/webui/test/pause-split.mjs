...
{
  /*
    🔴 語の時刻に間が無くても、エンジンが音で確かめた息継ぎ（breakAfter）では割る。
       Whisper の語の時刻は隣と隙間なく繋がるので、実素材では間が時刻に
       現れない（全語 0.00秒、2026-09-11）。言われた例:
         「今日は、(息継ぎ)勉強しようと思います」→「今日は」「勉強しようと思います」
  */
  const t = speech([['今日は', 0], ['勉強しようと思います', 0]])
  // 時刻は隙間なし。印だけ付ける
  t.words[2].breakAfter = true
  const texts = splitTelops([t], 26).map((x) => x.text)
  check('時刻に間が無くても、息継ぎの印で割る', texts.length === 2, texts.join(' / '))
  check('割った中身が正しい',
        texts[0] === '今日は' && texts[1] === '勉強しようと思います', texts.join(' / '))
  const times = splitTelops([t], 26).map((x) => `${x.start}-${x.end}`)
  check('割った先の時刻は語の時刻から出る', times[0] === '0-0.36' && times[1] === '0.36-1.56', times.join(' / '))
}
