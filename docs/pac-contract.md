...
2026-09-11 に、プラグイン版で踏んだ分をデスクトップ版（`sidecar/fcpxml.py`、
`src/timeline/fcpxml.ts` の形式名）へ写した:

- 形式の名前は「決まった大きさ・コマ数」のときだけ名乗る。縦の素材に
  `FFVideoFormat1920p30` のような、ありそうな名前を付けない（0.316 倍で真ん中に出る）
- コマ数は 1% 以内なら標準の値に寄せる（59.99 のままだとプロジェクトがカスタム扱い）
- `<asset>` に `format` を書かない（読みが食い違うと映像が半分の大きさで出る）
- テロップは「いちばん長く乗っているクリップ」にぶら下げる（始まりだけ見ると落ちる）
- 位置と長さはフレーム（整数）で持ち回す。シーケンスの尺は残したクリップの合計
- 速度を変えるときは `timeMap` を先頭に書き、ぶら下げたものの位置も出来上がりの時刻で測る
- 何で作ったかを1行、コメントで残す（`<!-- PAC / v… / 素材 … -->`）
