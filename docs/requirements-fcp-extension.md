...
| 部分 | 状態 |
|---|---|
| 拡張がビルドできる | ✅ CI で毎回ビルド・アドホック署名・関門チェックまで通る |
| パネルの中身 | ✅ WKWebView に React の UI を載せて `.appex` に同梱（関門で存在を検査） |
| カット画面 | ✅ プレビュー再生・波形タイムライン・候補の承認/却下・キーボード操作 |
| テロップ画面 | ✅ 左プレビュー / 右一覧・選択で同期・Cmd+C/V/D でコピペ複製・通常/強調の既定スタイル |
| FCP から読む情報 | ✅ アプリ名/バージョン/シーケンス名/長さ/fps/再生ヘッド位置 |
| フォント一覧 | ✅ macOS から取得（日本語フォントを上に寄せる） |
| 素材フォルダの許可 | ✅ 初回のみ。security-scoped bookmark で記憶 |
| FCPXML 書き出し | ✅ 1.13 固定・フレーム境界スナップ・カット済み spine + タイトル |

## まだ無いもの

- **解析エンジン**（音声から無音・フィラー・言い直しを見つけ、文字起こしする部分）。
  現状パネルは FCP から読んだ本物の情報だけを表示し、カット候補とテロップは空。
- **テロップテンプレの丸写し**（友達の「基本01_10」の effect uid 取り込み）。
  現状は Basic Title で出力するため、見た目は友達のテンプレと一致しない。
- ドラッグ&ドロップでの投入（現状は .fcpxml を書き出して FCP に読み込ませる）。

## エンジンをどこに置くかは、友達のテスト結果を見てから決める

パネルが登録される → 拡張はサンドボックス内なので、
**コンテナアプリ側でエンジンを動かし、ローカルソケットで繋ぐ**構成にする。
パネルが登録されない → 案A に切り替え、この UI は PAC とは別のアプリとして使う
（UI は WKWebView にも普通のブラウザにも載るので、どちらでも捨てずに済む）。

---

# 解析エンジン (2026-08-25)

## 方針: PAC 本体のロジックを **import して使う**（コピーしない）

無音・フィラー・言い直しの検出、幻聴の除去、テロップの組み立て、文字起こしの VAD 設定は
PAC 本体（`sidecar/`）で実測して詰めたもの。コピーすると必ず片方が腐るので import する。
**PAC 側には一切手を入れない（読むだけ）。**

`fcp-extension/engine/` が自前で持つのは2つだけ:

- `waveform.py` — ffmpeg で 16kHz モノラルに落として、タイムラインに描く振幅列を作る
- `mapping.py` — 検出結果をパネル（webui）が読む形に変換する（`note` は `normal` に寄せる）

## 実素材での通し確認（2026-08-25、Windows + RTX 5070 Ti）

18秒の実素材で `python -m pac_fcp_engine` を実行:

```
{"ok": true, "cuts": 4, "telops": 5, "duration": 18.068}
無音 2.80-3.68 / 6.88-8.26 / 11.12-13.80 / 16.40-18.07
テロップ 5件（うち1件が強調）
```

その JSON を UI に流し込み、ブラウザで確認:

- 実素材の動画が再生され、再生位置が進む
- カット候補4件がタイムラインと一覧に並ぶ
- 「全部承認」で「短くなる長さ 約 6.6 秒」（4区間の合計と一致）
- テロップを選ぶとプレビューに乗る（強調は黄色）

## 検査（Mac が無いので CI に見張らせる）

- `engine/tests/test_engine.py` — 14件。モデルも ffmpeg も要らない。
  作り物の文字起こしを通して「検出 → パネル用 JSON」まで確かめる
- `scripts/check_fcpxml.sh` — 19件。生成した FCPXML の版・時刻のフレーム境界・
  テンプレの写し・カットの反映・エスケープを確かめ、xmllint も通す

## まだ無いもの

- **エンジンを Mac で動かす形にする**（PyInstaller で1本にまとめ、コンテナアプリに同梱）
- **拡張 ⇔ エンジンの接続**（サンドボックス内からは直接起動できないのでローカルソケット）
- 初回のモデル取得（large-v3-turbo で約1.6GB）の扱い
...
```
① 動画を選ぶ
② 設定（言語 / 文字起こしの精度 / テロップの見本）  ← ここで「テロップ作成開始」
③ 解析（文字起こし → カット候補とテロップを同時に作る）
④ カットを確認
⑤ テロップを確認
⑥ FCPXML を書き出す
```
...
**フィラー（「えー」）と言い直しは「何と言ったか」が分からないと判定できない。**
無音だけは音から分かるが、3種類そろえるには文字起こしが先に要る。
よってカット候補は文字起こしの後にしか作れず、1回の解析で両方作る。

（もし「無音カットだけ先に確認 → その後に文字起こし」にしたい場合は、
解析を2段階に分ける必要がある。カット段階で出せるのは無音のみになる代わりに、
切った分だけ文字起こしが短くなって速い。今は採用していない。）

## モデルの選択

初回のダウンロード量を必ず画面に出す。何分も無反応に見えると壊れたと思われるため。

| 選択肢 | モデル | 初回のダウンロード |
|---|---|---|
| 高い（おすすめ） | large-v3-turbo | 1.6GB |
| ふつう | medium | 1.5GB |
| 低い（速い） | small | 480MB |
| 最低（試し用） | base | 145MB |
...
```
[FCP の中]                              [FCP の外]
パネル(.appex / サンドボックス内)        PAC.app ── エンジン(whisper / ffmpeg)
   │  ①「この動画を解析して」               ▲
   └──── 127.0.0.1:47829 ────────────────┘
   ◀── ②進捗「文字起こし中 62%」
   ◀── ③結果の JSON（カット候補・テロップ）
...
- パネルはサンドボックスの中にいるので、Python も ffmpeg も起動できず、
  1.6GB のモデルを置く場所も自由にならない。だから解析は外側のアプリにやってもらう。
- Apple の正式な方法（App Groups / XPC）は **Team ID を要求する**ので、
  アドホック署名（$99/年を払わない）では使えない。
  ローカルソケットなら `com.apple.security.network.client` の許可だけで済む。
- 動画そのものは渡さない。**パスだけ渡して、実際に読むのはサンドボックス外のエンジン**。
- 通信は 127.0.0.1（自分自身）なので、外のネットには一切出ない。

実装状況: パネル側（`EngineClient.swift`）は実装済み。
**エンジン側（PAC.app に置くサーバーと、Mac 向けにまとめたエンジン本体）は未実装。**
